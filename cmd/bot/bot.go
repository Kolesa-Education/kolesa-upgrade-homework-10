...
func (bot *UpgradeBot) StartHandler(ctx telebot.Context) error {
	...
	return ctx.Send("Привет, " + ctx.Sender().FirstName + "!\nЭто бот-планировщик!\nИспользуй команды /addTask, /showTasks, /deleteTask. Выбери любую из команд, чтобы узнать как она работает.")
}
...
func (bot *UpgradeBot) AddTaskHandler(ctx telebot.Context) error {
	...
	if allText == "/addTask" {
		return ctx.Send("Введи заголовок, описание и дату задачи. Разделяйте их символом '@'. \nНапример: \n/addTask Сделать ремонт@ На кухне и в спальне@01.01.2023")
	}
...
func (bot *UpgradeBot) DeleteTaskHandler(ctx telebot.Context) error {
	...
	return ctx.Send("Такой задачи больше нет")
...
func (bot *UpgradeBot) ShowTasksHandler(ctx telebot.Context) error {
	...
	for _, el := range tasks {
		task_info = fmt.Sprintf("ID: %d \ntitle: %s \ndesc: %s \ndeadline: %s", el.ID, el.Title, el.Description, el.EndDate.Format("02.01.2006"))
